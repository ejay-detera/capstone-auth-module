  ...
  <div class="space-y-6">
    ...
    <div v-if="showAssignModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div class="relative bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div class="p-5">
          <div v-if="loadingRoles" class="py-8 flex flex-col items-center justify-center gap-4 text-slate-500">
            <Loader2 class="animate-spin" :size="32" />
          ...
          <div v-else class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[35vh] overflow-y-auto pr-1 custom-scrollbar">
              <label 
                v-for="role in roles" 
                :key="role.id"
                class="relative flex items-center gap-2 p-2.5 rounded-xl border-2 transition-all cursor-pointer group"
              ...
              >
                ...
                >
                  <Check v-if="selectedRoleIds.includes(role.id)" :size="10" stroke-width="3" />
                </div>
                <input 
                  type="checkbox" 
                  :value="role.id" 
                ...
                <span class="text-xs font-bold text-slate-900 leading-tight">{{ role.name }}</span>
              </label>
            </div>
            ...
            <div class="pt-2 flex gap-3">
              <button 
                ...
                class="flex-1 px-6 py-2.5 text-sm font-semibold text-gray-600 border border-gray-300 hover:bg-gray-100 rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button 
                ...
                :disabled="submitting"
                class="flex-1 px-6 py-2.5 bg-gradient-to-r from-[#252578] to-[#3b82f6] text-white text-sm font-semibold rounded-xl hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
        ...
        <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
          ...
          <div class="pt-4 flex gap-3">
            <button 
              ...
              class="flex-1 px-6 py-3 text-sm font-semibold text-gray-600 border border-gray-300 hover:bg-gray-100 rounded-xl transition-colors"
            >
              Cancel
            </button>
            <button 
